...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        List<UserRole> userRoles = jdbcTemplate.query("SELECT user_id,role FROM user_roles",
                (rs, rowNum) -> new UserRole(rs.getInt("user_id"),
                        Role.valueOf(rs.getString("role"))));
        Map<Integer, List<Role>> userRoleMap = userRoles.stream()
                .collect(Collectors.groupingBy(UserRole::getUserId, Collectors.mapping(UserRole::getRole, Collectors.toList())));
        List<User> users = jdbcTemplate.query("SELECT * FROM users ORDER BY name, email", ROW_MAPPER);
        users.forEach(u -> u.setRoles(userRoleMap.get(u.getId())));
        return users;
    }
